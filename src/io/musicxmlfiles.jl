...
testfile = "/home/chfin/Uni/phd/data/csapp/mozart-piano-sonatas/musicxml/sonata15-1.xml"
...
function attribs!(attr, state)
    for node in child_elements(attr)
        ...
        elseif name(node) == "transpose"
            ...
            state.trans_dia = state.trans_dia + 7 * octs
            state.trans_chrom = state.trans_chrom + 12 * octs 
        end
    end
end

# TODO: ties (needs state: keep tie-starting note, push when completed)
